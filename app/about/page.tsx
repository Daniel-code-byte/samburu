  ...
  return (
    <>
      ...
      <section className="section-pad">
        <div className="about-story-grid" style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div>
            <div className="eyebrow">Our Mission</div>
            <div className="teal-rule" />
            <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.4rem)', fontWeight: 300, marginBottom: '1.5rem' }}>Why we exist</h2>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              {VALUES.map((v) => (
                <div key={v.title} style={{ padding: '1.5rem', borderLeft: '3px solid var(--teal)', background: 'var(--teal-pale)' }}>
                  <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.15rem', fontWeight: 600, marginBottom: '0.5rem' }}>{v.title}</h3>
      ...
      <section className="section-pad" style={{ background: 'var(--sand)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
            ...
            <h2 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.4rem)', fontWeight: 300, marginTop: '0.5rem' }}>Meet the team</h2>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem', marginBottom: '3.5rem' }}>
            {TEAM.map((m) => (
              <div key={m.email} className="sw-card" style={{ padding: '2rem', textAlign: 'center' }}>
                <div style={{ width: 64, height: 64, borderRadius: '50%', background: m.color, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.25rem' }}>
                ...
                <thead>
                  <tr style={{ borderBottom: '2px solid var(--border-md)' }}>
                    {['Role', 'Primary Email', 'Official Alias'].map((h) => (
                      <th key={h} style={{ textAlign: 'left', padding: '0.9rem 1.25rem', fontSize: '0.72rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--muted)', fontWeight: 600, fontFamily: 'DM Sans, sans-serif' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
      ...
      <section style={{ background: 'var(--teal-deep)', padding: '2rem' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
          <p style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.35)' }}>Website developed by Daniel Lepati</p>
